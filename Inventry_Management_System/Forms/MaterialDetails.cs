...
namespace Inventry_Management_System.Masters
{
    public partial class MaterialDetails : Form
    {
        ...
        private void MaterialDetails_Load(object sender, EventArgs e)
        {
            ...
            DGV_MaterialDetails.Columns[2].HeaderText = "Material Price";
            DGV_MaterialDetails.Columns[2].Name = "Material Price";
        ...
        public DataTable ListData()
        {
            ...
            if (cn.MakeConnection() != false)
            {
                try
                {
                    MySqlDataAdapter cmd = new MySqlDataAdapter("SELECT * FROM material_master where Action!='N'", cn.connection);
                    cmd.Fill(dt);
                }
                catch (Exception ex)
                {
        ...
        public DataTable ListSearchData(string SearcherText)
        {
            ...
            if (cn.MakeConnection() != false)
            {
                try
                {
                    MySqlDataAdapter cmd = new MySqlDataAdapter("SELECT * FROM material_master WHERE Action!='N' and Material_Name LIKE '%" + SearcherText + "%'", cn.connection);
                    cmd.Fill(dt);
                }
                catch (Exception ex)
                {
        ...
        public void DeleteData(string Flag, string Id)
        {
            if (cn.MakeConnection() != false)
            {
                try
                {
                    ...
                    MySqlCommand cmd = new MySqlCommand("UPDATE material_master SET Action = @Action where Id = " + Id, cn.connection);
